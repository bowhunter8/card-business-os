...
import TaxExportButton from '../tax/TaxExportButton'
import TaxPdfExportButton from '../tax/TaxPdfExportButton'
...
export default async function TaxReportPage({
  ...
  const cpaPacketQueryString = `year=${selectedYear}&period=year`
  const cpaPacketPdfHref = `/api/reports/pdf/cpa-packet?${cpaPacketQueryString}`
  const cpaPacketCsvHref = `/api/reports/cpa-packet?${cpaPacketQueryString}`
      ...
      <div className="app-page-header gap-4">
        <div>
          ...
          <p className="app-subtitle">
            Year-end tax summary, Schedule C settings, inventory carryover, tax readiness review, and CPA packet exports.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <a href={cpaPacketPdfHref} className="app-button-primary">
            Download CPA Packet PDF
          </a>
        </div>
      </div>
      ...
      <div className="app-section p-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-xl font-semibold text-zinc-100">
              CPA Packet for {selectedYear}
            </h2>
            <p className="mt-1 max-w-4xl text-sm text-zinc-400">
              Generate a consolidated accountant-facing packet from the Year-End Tax Center. The PDF is the printable review packet; the CSV packet is the spreadsheet backup for deeper review. The Tax Center remains the control panel for warnings, Schedule C settings, inventory carryover, and final review.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <a href={cpaPacketCsvHref} className="app-button">
              Download CPA CSV Packet
            </a>

            <a href={cpaPacketPdfHref} target="_blank" rel="noreferrer" className="app-button">
              Open / Print Packet
            </a>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950/40 p-4">
            <h3 className="font-semibold text-zinc-100">Schedule C Support</h3>
            <p className="mt-2 text-sm text-zinc-400">
              Gross sales, realized COGS, selling costs, expenses, and extra Schedule C entries.
            </p>
          </div>

          <div className="rounded-2xl border border-zinc-800 bg-zinc-950/40 p-4">
            <h3 className="font-semibold text-zinc-100">Inventory Basis</h3>
            <p className="mt-2 text-sm text-zinc-400">
              Beginning inventory, ending inventory, available inventory cost basis, and carryover support.
            </p>
          </div>

          <div className="rounded-2xl border border-zinc-800 bg-zinc-950/40 p-4">
            <h3 className="font-semibold text-zinc-100">Sales / Expenses</h3>
            <p className="mt-2 text-sm text-zinc-400">
              Sales detail, platform costs, manual expenses, category summaries, and accountant backup.
            </p>
          </div>

          <div className="rounded-2xl border border-zinc-800 bg-zinc-950/40 p-4">
            <h3 className="font-semibold text-zinc-100">Review Warnings</h3>
            <p className="mt-2 text-sm text-zinc-400">
              Missing beginning inventory, unlocked ending inventory, uncategorized expenses, and disposal review items.
            </p>
          </div>
        </div>

        {taxReadinessWarnings.length > 0 ? (
          <div className="mt-4 rounded-2xl border border-amber-700/70 bg-amber-950/30 p-4 text-sm text-amber-100">
            <div className="font-semibold">Review before sending to a CPA:</div>
            <ul className="mt-3 list-disc space-y-2 pl-5">
              {taxReadinessWarnings.map((warning) => (
                <li key={warning}>{warning}</li>
              ))}
            </ul>
          </div>
        ) : (
          <div className="mt-4 rounded-2xl border border-emerald-700/70 bg-emerald-950/30 p-4 text-sm text-emerald-100">
            No major tax readiness warnings were detected from the current tracked data. Still review records, receipts, and marketplace reports before filing.
          </div>
        )}
      </div>
