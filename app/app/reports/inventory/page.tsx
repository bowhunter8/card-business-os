...
function resolveReportDateRange(params: SearchParams) {
  ...
  const explicitStart = params.startDate || params.dateFrom || ''
  const explicitEnd = params.endDate || params.dateTo || ''
    ...
    <main className="app-page space-y-4">
      ...
      <section className="app-section space-y-3">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          ...
          <Link href={buildInventoryHref(search, selectedStatus)} className="app-button" prefetch={false}>
            Open Inventory
          </Link>
        </div>
