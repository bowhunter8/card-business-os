...
function isSectionHeader(row: SheetRow) {
  ...
  if (!first || isCountRow(first)) return false
...
function looksLikeCardNumber(value: string) {
  ...
  return /^(?:#?\d{1,4}|[A-Z0-9]{1,8}-[A-Z0-9]{1,8})$/i.test(cleaned)
}
...
function suggestedMappingsFromRows(rows: SheetRow[]) {
  ...
  for (const row of sample) {
    for (let index = 0; index < width; index += 1) {
      ...
      if (looksLikeCardNumber(value)) scores[index].card += 4
      if (looksLikeBaseballTeam(value)) scores[index].team += 10
      if (looksLikePersonName(value)) scores[index].player += 3
  ...
  // Team must be detected before Player. Both are usually multi-word text,
  // which made the older statistical guesser capable of swapping them.
  // Team recognition now relies on real baseball team/country labels while
  // preserving the original printed text in the imported checklist.
  const cardNumber = best('card')
  const printedTeam = best('team', compactIndices(cardNumber))
  const playerName = best('player', compactIndices(cardNumber, printedTeam))
  const rookieFlag = best(
    'rookie',
    compactIndices(cardNumber, printedTeam, playerName)
  )

  // Checklist Insider sectioned exports normally resolve to card number /
  // player / team / rookie marker. The fallbacks remain for known files, but
  // recognized team values take priority when a workbook changes column order.
  return {
    sectionName: null,
...
function validateChecklistInsiderTeams(parsed: {
  ...
  const teamValues = parsed.items
    .map((item) => text(item.printedTeam))
    .filter(Boolean)

  const uniqueTeams = Array.from(new Set(teamValues.map(normalizeTeamDetectionValue)))

  if (teamValues.length === 0) {
    throw new Error(
      'HITS recognized a Checklist Insider workbook but could not identify its Team column safely. Nothing was imported.'
    )
  }

  const recognizedTeamRows = teamValues.filter(looksLikeBaseballTeam).length
  const recognizedRatio = recognizedTeamRows / teamValues.length

  // A baseball checklist can legitimately include historical franchises and
  // WBC countries, but hundreds of unique "teams" is a strong indication that
  // the Player column was mistaken for Team. Stop before writing bad data.
  if (uniqueTeams.length > 100 || recognizedRatio < 0.35) {
    throw new Error(
      `HITS recognized a Checklist Insider workbook, but the Team column did not pass validation (${uniqueTeams.length} unique values). Nothing was imported.`
    )
  }
}
...
async function handleBeckettImport(params: {
  ...
  const { uploaded, supabase, userId } = params
  const stats = newStats()
...
function looksLikeChecklistInsiderRows(rows: SheetRow[]) {
  ...
  return sectionRows >= 1 && cardRows >= 5
}
...
async function handleChecklistInsiderImport(params: {
  ...
  const { uploaded, buffer, supabase, userId } = params
  const stats = newStats()
  const rawRows = await readSheet(buffer)
  const rows: SheetRow[] = rawRows.map((row) =>
    row.map((value) => {
  ...
  const metadata = inferChecklistInsiderMetadata(uploaded.name)
  const persisted = await persistChecklist({
    supabase,
    userId,
    uploadedName: uploaded.name,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // its known sectioned card-list structure.
