...
function getRowsForBackupKey(
  ...
  if (typeof raw === 'object' && raw !== null && !Array.isArray(raw)) {
    const legacyRows = (raw as { rows?: unknown[] }).rows

    if (!legacyRows) return []

    if (!Array.isArray(legacyRows)) {
      throw new Error(`Backup section "${key}" rows are not an array.`)
    }

    return legacyRows.filter((row): row is Record<string, unknown> => {
      return Boolean(row && typeof row === 'object' && !Array.isArray(row))
    })
  }
...
async function getExistingIdsForRows(
  ...
  if (error) {
    throw new Error(
      `Could not check existing records for ${table}: ${error.message}`
    )
  }
...
async function insertRows(
  ...
  if (error) {
    throw new Error(
      `Could not insert rows into ${table}: ${error.message}`
    )
  }
...
async function deleteExistingUserRows(
  ...
  if (readError) {
    throw new Error(
      `Could not read existing rows from ${table}: ${readError.message}`
    )
  }
  ...
  if (deleteError) {
    throw new Error(
      `Could not delete existing rows from ${table}: ${deleteError.message}`
    )
  }
...
async function restoreSnapshot(
  ...
  for (const item of [...snapshot].reverse()) {
    ...
    if (error) {
      throw new Error(
        `Rollback failed for ${item.table}: ${error.message}`
      )
    }
  }
}

async function createEmergencyRestorePoint(
  supabase: Awaited<ReturnType<typeof createClient>>,
  userId: string
) {
  try {
    const restorePoint = {
      backup_type: 'hits_emergency_restore_point',
    ...
    await supabase.from('backup_restore_points').insert({
      user_id: userId,
      backup_name: `Emergency Backup ${new Date().toLocaleString()}`,
      backup_type: 'emergency',
      backup_json: restorePoint,
    })
  } catch {
    // Do not block restore if emergency restore point storage fails.
  }
}
...
async function runMergeRestore({
  ...
  for (const tableConfig of BACKUP_TABLES) {
    const rawRows = getRowsForBackupKey(
      backupTables,
      tableConfig.key
    )

    const rows = sanitizeRowsForUser(
      rawRows,
      userId,
      tableConfig.userOwned
    )
...
export async function POST(request: NextRequest) {
  ...
  const mode: RestoreMode = body.mode ?? 'replace'
  ...
  if (body.confirmed !== true) {
    return jsonError(
      'Restore must be confirmed before it can run.'
    )
  }
  ...
  try {
    await createEmergencyRestorePoint(
      supabase,
      user.id
    )
